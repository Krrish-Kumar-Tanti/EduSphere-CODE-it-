...
export default function GrievanceDrawer() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-3xl bg-rose-50 border border-rose-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xs">
        ...
        <span className="text-[11px] font-bold px-3 py-1 rounded-xl bg-white border border-rose-200 text-rose-700 shadow-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative">
            ...
            {showSuccessToast && (
              <div className="mb-4 p-3.5 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold flex items-center justify-between animate-fadeIn">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`p-3.5 rounded-2xl border text-left flex items-start gap-2.5 transition ${
                      destination === 'hod'
                        ? 'bg-purple-50 border-2 border-purple-600 ring-2 ring-purple-500/10 text-slate-900'
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-slate-500 font-medium">Curriculum, faculty pace, leaves</span>
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-2xl border text-left flex items-start gap-2.5 transition ${
                      destination === 'staff'
                        ? 'bg-amber-50 border-2 border-amber-600 ring-2 ring-amber-500/10 text-slate-900'
                        : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100'
                    }`}
                  >
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-slate-500 font-medium">AC, cleaning, anti-bullying, medical</span>
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:bg-white focus:border-indigo-500 focus:outline-none font-medium placeholder-slate-400"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 focus:bg-white focus:border-indigo-500 focus:outline-none font-medium placeholder-slate-400 leading-relaxed"
                />
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <label className="cursor-pointer px-4 py-2 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 flex items-center gap-2 transition font-medium">
              ...
              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <div className={`p-2 rounded-xl ${isAnonymous ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-200 text-slate-500'}`}>
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-xs transition"
              >
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 text-[11px]">
              <button 
                ...
                className={`px-2 py-0.5 rounded-md ${filterCategory === 'all' ? 'bg-slate-900 text-white font-bold' : 'text-slate-500 font-medium'}`}
              >
                All
              </button>
              <button 
                onClick={() => setFilterCategory('hod')} 
                className={`px-2 py-0.5 rounded-md ${filterCategory === 'hod' ? 'bg-purple-100 text-purple-800 font-bold' : 'text-slate-500 font-medium'}`}
              >
                HOD
              </button>
              <button 
                onClick={() => setFilterCategory('staff')} 
                className={`px-2 py-0.5 rounded-md ${filterCategory === 'staff' ? 'bg-amber-100 text-amber-800 font-bold' : 'text-slate-500 font-medium'}`}
              >
                Staff
              </button>
            </div>
          ...
          <div className="space-y-3 max-h-[560px] overflow-y-auto pr-1">
            {filteredGrievances.map((g) => (
              <div key={g.id} className="p-4 rounded-2xl bg-white border border-slate-200 shadow-xs text-xs space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="text-[10px] font-mono text-indigo-700 font-bold bg-indigo-50 px-2 py-0.5 rounded border border-indigo-200">{g.id}</span>
                    <h5 className="font-bold text-slate-900 text-sm mt-1">{g.title}</h5>
                  </div>
                ...
                <p className="text-slate-600 text-xs leading-relaxed font-medium">{g.description}</p>
