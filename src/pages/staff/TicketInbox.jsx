...
export default function TicketInbox({ activeDomain, setActiveDomain }) {
  const { grievances, updateGrievanceStatus, addGrievance } = useData();

  const [statusFilter, setStatusFilter] = useState('All'); // 'All' | 'Urgent' | 'In-Progress' | 'Resolved'
  ...
  const filteredTickets = staffTickets.filter(ticket => {
    ...
    const matchesStatus = 
      statusFilter === 'All' ||
      (statusFilter === 'Urgent' && (ticket.priority === 'Urgent' || ticket.priority === 'High')) ||
                  ...
                  {!isResolved ? (
                    ...
                    <div className="flex items-center justify-between text-[11px] text-emerald-700 font-bold">
                      <span className="flex items-center gap-1">
                        <CheckCircle2 className="w-4 h-4 text-emerald-600" /> Issue Closed & Verified
                      </span>
                      <span className="text-slate-400 font-mono">Archive #REC-{ticket.id}</span>
                    </div>
                  )}
